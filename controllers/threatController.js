...
export const saveIpRule = async (req, res) => {
  try {
    ...
    if (!ip || !listType) {
      return res.status(400).json({ message: "IP and listType are required" });
    }

    if (!["blacklist", "whitelist"].includes(listType)) {
      return res.status(400).json({ message: "listType must be blacklist or whitelist" });
    }

    const record = await ThreatIPList.findOneAndUpdate(
      { ip },
      { ip, listType, reason: reason || "Manual access control rule added" },
      { new: true, upsert: true }
    );

    // LOG: manual IP rule
    await createLog(
      listType === "blacklist" ? "suspicious" : "info",
      `IP ${ip} manually added to ${listType}: ${reason || "Manual access control rule"}`,
      ["threat", listType, "manual"],
      ip, "Admin"
    );

    res.json({ success: true, message: "IP rule saved successfully", data: record });
  } catch (err) {
    console.error("saveIpRule error:", err.message);
    res.status(500).json({ message: "Failed to save IP rule" });
  }
};
...
export const detectThreat = async (req, res) => {
  try {
    ...
    const { url = "/", method = "GET", payload = "", failed = false } = req.body;

    const listedIp = await ThreatIPList.findOne({ ip });

    if (listedIp?.listType === "whitelist") {
      return res.json({ success: true, bypassed: true, message: "Whitelisted IP allowed", ipRule: listedIp });
    }

    if (listedIp?.listType === "blacklist") {
      const threat = await Threat.create({
        ip, url, method, payload,
        attackType: "suspicious-ip",
        confidence: 100, threatScore: 100,
        threatLevel: "HIGH", action: "block",
        requestCount: 0, failedAttempts: 0,
        reason: "IP already exists in blacklist",
      });

      emitThreatAlert(threat);

      await createLog("suspicious", `Blacklisted IP attempted access: ${ip} on ${url}`, ["threat", "blacklist", "blocked"], ip, req.headers["user-agent"] || "Unknown");

      return res.status(403).json({ success: false, message: "Blocked blacklisted IP", threat });
    }
    ...
    if (activity.requestCount >= RATE_LIMIT_THRESHOLD) {
      ...
      aiResult = { attackType: "rate-limit", confidence: 100, threatScore: 100, threatLevel: "HIGH", action: "block", reason: autoBlockReason };
    }
    ...
    if (activity.failedAttempts >= FAILED_ATTEMPT_THRESHOLD) {
      ...
      aiResult = { attackType: "suspicious-ip", confidence: 100, threatScore: 100, threatLevel: "HIGH", action: "block", reason: autoBlockReason };
    }

    if (aiResult.attackType !== "normal" && aiResult.confidence >= AUTO_BLOCK_CONFIDENCE && aiResult.threatLevel === "HIGH") {
      autoBlocked = true;
    ...
    if (autoBlocked) await autoBlacklistIp(ip, autoBlockReason);
    ...
    if (threat.threatLevel === "HIGH" || threat.action === "block") {
      emitThreatAlert(threat);
      // LOG: high threat detected
      await createLog(
        "suspicious",
        `AI Threat: ${aiResult.attackType} from ${ip} (confidence: ${aiResult.confidence}%)${autoBlocked ? " — IP auto-blacklisted" : ""}`,
        ["threat", "ai-detection", aiResult.attackType.replace(/\s+/g, "-")],
        ip, req.headers["user-agent"] || "Unknown",
        { attackType: aiResult.attackType, confidence: aiResult.confidence, autoBlocked }
      );
    }

    return res.json({ success: true, autoBlocked, threat });
  } catch (err) {
    console.error("detectThreat error:", err.message);
    return res.status(500).json({ message: "Threat detection failed" });
  }
};
...
export const getThreatStats = async (req, res) => {
  try {
    const [total, high, medium, low, blocked, monitored, blacklisted, whitelisted] = await Promise.all([
    ...
    res.json({ total, high, medium, low, blocked, monitored, blacklisted, whitelisted });
  } catch (err) {
...
export const getIpLists = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to fetch IP lists" });
  }
};
...
export const deleteIpFromList = async (req, res) => {
  try {
    ...
    if (!item) return res.status(404).json({ message: "IP rule not found" });

    // LOG: IP removed
    await createLog("info", `IP ${item.ip} removed from ${item.listType}`, ["threat", item.listType, "removed"], item.ip, "Admin");

    res.json({ success: true, message: "IP removed from list" });
  } catch (err) {
...
export const deleteThreat = async (req, res) => {
  try {
    ...
    if (!threat) return res.status(404).json({ message: "Threat record not found" });
...
export const bulkDeleteThreats = async (req, res) => {
  try {
    const { ids } = req.body;
    if (!Array.isArray(ids) || ids.length === 0) return res.status(400).json({ message: "No threat IDs provided" });
    const result = await Threat.deleteMany({ _id: { $in: ids } });
    res.json({ success: true, message: `${result.deletedCount} threat records deleted`, deletedCount: result.deletedCount });
  } catch (err) {
    console.error("bulkDeleteThreats error:", err.message);
    res.status(500).json({ message: "Failed to delete selected threats" });
  }
};
...
const writeThreatPdf = (doc, threat, index = null) => {
  ...
  doc.text(`Created At: ${threat.createdAt ? new Date(threat.createdAt).toLocaleString() : "-"}`);
  doc.moveDown(1);
};
...
export const exportThreatsPdf = async (req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", "attachment; filename=ai-threat-analysis-report.pdf");

    const doc = new PDFDocument({ margin: 40, size: "A4" });
    doc.pipe(res);

    doc.fontSize(22).fillColor("#0EA5E9").text("AI Threat Detection Report", { align: "center" });
    ...
    res.status(500).json({ message: "Failed to export threats PDF" });
  }
};
...
export const exportSingleThreatPdf = async (req, res) => {
  try {
    ...
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename=threat-${threat.ip || "record"}.pdf`);

    const doc = new PDFDocument({ margin: 40, size: "A4" });
    doc.pipe(res);

    doc.fontSize(22).fillColor("#0EA5E9").text("Single Threat Analysis Report", { align: "center" });
    doc.fontSize(10).fillColor("#6B7280").text(`Generated: ${new Date().toLocaleString()}`, { align: "center" });
    doc.moveDown(2);
    ...
    res.status(500).json({ message: "Failed to export single threat PDF" });
  }
};