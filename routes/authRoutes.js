...
router.post("/login", async (req, res) => {
  try {
    ...
    // 🔹 NEW: send OTP on successful login (email + OTP)
    const code = Math.floor(100000 + Math.random() * 900000).toString();
    await OTP.deleteMany({ email });
    ...
    const mailOptions = {
      ...
      text: `Your one-time login code is: ${code}. It will expire in 5 minutes.`
    };

    await transporter.sendMail(mailOptions);

    // Log successful login (same as pehle)
    ...
    // ⚠️ Response structure same + extra flag
    res.json({ 
...
// ✅ OTP verify for login / google (NEW route, baki sab same)
...
router.post("/google", async (req, res) => {
  try {
    ...
    // 🔹 NEW: send OTP for Google login as well
    const code = Math.floor(100000 + Math.random() * 900000).toString();
    await OTP.deleteMany({ email });
    await OTP.create({ email, code });
    ...
    const mailOptions = {
      ...
      text: `Your one-time Google login code is: ${code}. It will expire in 5 minutes.`
    };
